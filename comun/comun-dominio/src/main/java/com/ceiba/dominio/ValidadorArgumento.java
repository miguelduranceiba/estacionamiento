...
public class ValidadorArgumento {
    ...
    public static void validarRegex(String valor, String regex, String mensaje) {
    ...
    public static <T> void validarEnumerado(Predicate<T> filter, Class<T> enumAObtener, String mensaje) {

        Optional resultadoOpcional = Arrays.stream(enumAObtener.getEnumConstants())
                .filter(filter).findFirst();

        if (resultadoOpcional.isPresent()) {
            resultadoOpcional.get();
        } else {
            throw new ExcepcionValorInvalido(mensaje);
        }
    }
    ...
    public static void validarNumeroYLetra(String valor, String mensaje) {
        validarRegex(valor, "^[A-Za-z0-9]*", mensaje);
    }
}
